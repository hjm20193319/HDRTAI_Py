# [개념] Random Forest 분류 모델
# [개념] 다항 분류(Multinomial Classification): 종속변수의 범주가 3개 이상인 경우 사용함.
# [개념] Random Forest: 여러 개의 의사결정 나무를 결합하여 예측력을 높이는 앙상블(Bagging) 기법.
# [개념] 데이터의 균일도(Gini, Entropy)를 기준으로 무작위 샘플링된 트리를 생성하여 투표를 통해 분류함.
# [추천] n_estimators(트리 개수)와 max_depth를 조절하여 모델의 성능과 과적합을 제어해야 함.
...
model = RandomForestClassifier(criterion='gini', n_estimators=500, random_state=0) # [문법] RandomForestClassifier: n_estimators(결정트리 개수)를 설정하여 앙상블 모델 생성.
...
joblib.dump(model, 'rfmodel.pkl')     # [문법] dump(): 학습된 모델을 파일로 저장.
del model
read_model = joblib.load('rfmodel.pkl') # [문법] load(): 저장된 모델 파일을 읽어와 객체로 복원.
...
print(read_model.predict_proba(new_data)) # [문법] predict_proba(): 각 클래스(0, 1, 2)에 속할 확률값을 반환.
print('\n')
...
############################################
# 중요 변수 확인하기
print('특성 (변수) 중요도 : ', read_model.feature_importances_) # [문법] feature_importances_: 각 특성이 불순도 감소에 기여한 정도를 수치로 표현.
############################################
...
x_combined_std = np.vstack((x_train, x_test)) # [문법] vstack(): 학습 데이터와 테스트 데이터를 수직으로 쌓음.
