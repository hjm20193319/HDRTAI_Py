...
# [개념] SVM: 데이터들 사이의 거리를 최대화하는 결정 경계(Hyperplane)를 찾아 분류하는 알고리즘.
# [개념] 커널 트릭(Kernel Trick): 저차원 데이터를 고차원으로 매핑하여 비선형 분리를 가능하게 함.
# [추천] 데이터의 스케일에 민감하므로 StandardScaler 등을 이용한 표준화 전처리를 권장함.
...
# [문법] SVC: SVM을 이용한 분류 모델 생성. C(규제 파라미터)와 kernel 종류를 설정 가능.
model = svm.SVC(C=1, kernel='linear', random_state=0)

# from lightgbm import LGBMClassifier
# model = LGBMClassifier(boosting_type='gbdt', n_estimators=500, random_state=0, verbose=-1) 
...
joblib.dump(model, 'svmmodel.pkl')     # sav, model, ...
del model
...
############################################
# 중요 변수 확인하기
# [주의] 선형 커널(linear)인 경우에만 coef_를 통해 변수 영향력 확인 가능
print('특성 (변수) 계수(coef_) : ', read_model.coef_)
############################################
