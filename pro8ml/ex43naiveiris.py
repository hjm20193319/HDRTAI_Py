...
# [개념] 다항 분류(Multinomial Classification): 종속변수의 범주가 3개 이상인 경우 사용함. (Setosa, Versicolor, Virginica 3종 분류)
# [개념] GaussianNB: 특성들이 정규 분포(가우시안 분포)를 따른다고 가정하고 베이즈 정리를 적용하여 분류하는 알고리즘. (연속형 데이터 처리에 최적화)
# [개념] 조건부 확률: 각 클래스에 속할 확률을 계산하여 가장 높은 확률을 가진 클래스로 분류함. (P(A|B) 형태의 사후 확률 계산)
...
######################################
# 분류 모델 생성
# [문법] GaussianNB: 연속형 변수에 적합한 가우시안 나이브 베이즈 모델 생성. (각 특징이 정규분포를 따른다고 가정)
from sklearn.naive_bayes import GaussianNB
model = GaussianNB()

# [문법] fit(): 학습 데이터를 사용하여 각 클래스별 평균(theta_)과 분산(var_) 등을 계산함.
model.fit(x_train, y_train) # [문법] fit(): 학습 데이터를 사용하여 모델의 파라미터(평균, 분산 등)를 최적화함.

# 분류 예측
# [문법] predict(): 학습된 모델을 사용하여 테스트 데이터의 클래스(0, 1, 2)를 예측함. (사후 확률이 가장 높은 클래스 선택)
...
import joblib   # pickle보다 빠르고 대용량 데이터 처리에 효율적인 객체 직렬화 모듈
joblib.dump(model, 'nbmodel.pkl')     # 학습된 모델 객체를 파일로 저장 (확장자는 pkl, model 등 사용 가능)
del model
read_model = joblib.load('nbmodel.pkl') # 저장된 모델 파일을 읽어와 객체로 복원
...
# [문법] predict_proba: 각 클래스에 속할 확률(베이즈 확률)을 반환함. (합계는 항상 1.0)
...
############################################
# 클래스별 특성 평균 확인
# [개념] GaussianNB는 coef_ 대신 theta_를 통해 각 클래스별 피처의 평균값을 제공함.
print('클래스별 특성 평균(theta_) : \n', read_model.theta_)
# [개념] var_ : 각 클래스별 피처의 분산값
print('클래스별 특성 분산(var_) : \n', read_model.var_) # 데이터가 평균에서 얼마나 퍼져있는지 나타내는 지표
############################################
