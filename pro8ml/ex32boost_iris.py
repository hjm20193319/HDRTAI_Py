...
# [개념] Boosting: 약한 분류기에 대해 샘플의 일부를 보완해가며 순차적으로 학습해, 강한 분류기를 만듦.
# [추천] 데이터셋의 크기가 작을 경우 과적합(Overfitting)에 취약할 수 있으므로 하이퍼파라미터 조절이 중요함.
...
model = LGBMClassifier(boosting_type='gbdt', n_estimators=500, random_state=0, verbose=-1) # [문법] LGBMClassifier: LightGBM 분류 모델 생성.
...
joblib.dump(model, 'lgbmodel.pkl')     # sav, model, ...
del model
read_model = joblib.load('lgbmodel.pkl')
...
############################################
# 중요 변수 확인하기
print('특성 (변수) 중요도 : ', read_model.feature_importances_)
############################################
