...
# [개념] GaussianNB: 특성들이 정규 분포(가우시안 분포)를 따른다고 가정하고 베이즈 정리를 적용하여 분류하는 알고리즘.
# [개념] 조건부 확률: 각 클래스에 속할 확률을 계산하여 가장 높은 확률을 가진 클래스로 분류함.
...
######################################
# 분류 모델 생성
# [문법] KNeighborsClassifier: K-NN 분류 모델 생성. n_neighbors로 이웃의 개수를 설정.
from sklearn.neighbors import KNeighborsClassifier
model = KNeighborsClassifier(n_neighbors=5)

# [문법] fit(): 학습 데이터를 사용하여 이웃을 탐색할 수 있도록 데이터를 저장함.
...
joblib.dump(model, 'knnmodel.pkl')     # sav, model, ...
del model
read_model = joblib.load('knnmodel.pkl')
...
# [문법] predict_proba: 주변 이웃들의 비율을 바탕으로 각 클래스에 속할 확률을 반환함.
