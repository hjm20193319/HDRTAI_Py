...
# [개념] Softmax 함수: 각 클래스에 속할 확률의 총합이 1이 되도록 변환하여 가장 높은 확률을 가진 클래스를 선택함.
# [개념] LogisticRegression은 기본적으로 이항 분류 모델이나, OvR(One-vs-Rest) 또는 Multinomial 방식을 통해 다항 분류를 지원함.
# [추천] 독립변수 간 상관관계가 너무 높으면(다중공선성) 모델이 불안정해질 수 있으므로 사전에 확인이 필요함.
...
#   ㄴ 다중 클래스(label, 종속변수)를 지원하도록 일반화 됨
#   ㄴ 이를 softmax regression 또는 multinormial ligistic regression이라고 부른다
...
# # [개념] Scaling (데이터 크기 표준화 - 최적화 과정에서 안정성, 수렴속도 향상, 과적합/과소적합 방지 등의 효과)
# sc = StandardScaler()
# sc.fit(x_train) # 독립변수(feature)만 표준화 함
# sc.fit(x_test)
...
# # [문법] inverse_transform: 표준화된 데이터를 원래의 스케일로 복원함.
...
# iris data는 크기의 차이가 거의 없으므로 표준화는 의미 없다

######################################
# 분류 모델 생성
# [문법] LogisticRegression(): 사이킷런의 로지스틱 회귀 모델 생성. multi_class='auto'가 기본값이며 다항 분류를 자동으로 처리함.(softmax, sigmoid ...)
model = LogisticRegression(C=0.1, solver='lbfgs', multi_class='multinomial', random_state=0)   # 모델에 패널티 적용 ==> C : L2 규제 (숫자값을 조정해 가며 정확도 확인)
...
print('\n')
# softmax 의 출력값 중에서 가장 큰 확률 값의 인덱스를 반환 해줌
