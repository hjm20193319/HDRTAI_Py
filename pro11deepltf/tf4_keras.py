# 다층 신경망 구성 : keras 모듈 사용
#                       ↪ Deep Learning lib
# 일관성 있게 API를 제공 받을 수 있다
# 머신 러닝 모델을 쉽게 작성 가능

# 실습 : 논리 회로 처리를 위한 간단한 분류 모델 작성
import numpy as np
...
# loss 손실함수 : 훈련데이터에서 신경망의 성능을 측정하는 방법으로 모델이 정확히 학습할 수 있도록
# optimizer : 입력데이터와 손실함수를 기반으로 모델을 갱신함
# metrics : 훈련단계와 검정 단계를 모니터링 하기 위해 사용(모델 성능 지표)
# optimizer='sgd'
# optimizer=SGD(learning_rate=0.1)

# 4) 모델 학습 : train data 사용
#       ↪ 더 나은 결과를 찾는 자동화된 과정
...
proba = model.predict(x, verbose=1)
print('proba : \n', proba.ravel())
...
new_proba = mymodel.predict(x, verbose=1)
...
# 역전파 반대 방향으로 진행 : 손실 → 출력층 → 은닉층 → 입력층 방향

# 한마디로 역전파는 손실값을 기준으로 각 가중치의 책임 정도를 계산하고,
# 그 결과를 이용해 모델의 가중치를 수정하는 딥러닝 학습의 핵심 알고리즘이다