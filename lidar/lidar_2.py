...
import math
import numpy as np
import gymnasium as gym     # 실습 환경을 제공 → 현재상태 제공 → 행동 선택 가능 → 환경이 행동을 반영 → 새로운 상태, 보상, 종료 조건 반환
...
NUM_RAYS = 20
FOV = np.deg2rad(150)
MAX_RANGE = 8.0     # 라이다 최대 감지 거리(유효 사거리)
STEP_MARCH = 0.05   # ray 전진 단위 거리

def inside_worldFunc(x, y):     # 좌표값이 시뮬레이터 공간 경계 내부에 있는지 여부
    return 0.0 <= x <= WORLD_W and 0.0 <= y <= WORLD_H


# 라이다 광선의 끝점이 장애물과 충돌했는지 여부
def hit_circleFunc(px, py, cx, cy, r):     
    return (px - cx) ** 2 + (py - cy) ** 2 <= r ** 2    # 원 내부에 있으면 True반환


# 에이전트(x, y, theta) 에서 시야각(FOV)으로 NUM_RAYS개의 광선을 쏴,
# 각 ray가 처음 부딪히는 지점까지 거리, 각도 반환 함수
def cast_lidarFunc(x, y, theta, num_rays=NUM_RAYS, fov=FOV, max_range=MAX_RANGE, step=STEP_MARCH):
    start = theta - fov / 2     # 전방 시야각 왼쪽 끝 각도(첫번째 Ray 시작 각도)
    ...
    for i, ang in enumerate(angles):
        ...
        while dist < max_range:
            px = x + np.cos(ang) * dist     # Ray 끝점 x 좌표
            py = y + np.sin(ang) * dist     # Ray 끝점 y 좌표
...
class SimpleLidarEnv(gym.Env):
    def __init__(self, render_mode='human'):
        ...
        self.v = 0.25   # 전진 속도
        self.steer_delta = np.deg2rad(8)    # 회전각도 각 8도
        ...
        self.fig, self.ax = None, None  # 랜더링 용 객체
        self._state = None  # [x, y, theta]
        self._prev_goal_dist = None  # 이전 목표와의 거리
        self._steps = 0  # 행동 횟수
    ...
    def _get_obs(self):
        ...
        obs, _ = cast_lidarFunc(x, y, th)   # 라이다 거리 관측
        return obs.astype(np.float32)


    # Gymnasium 환경에 추가정보를 제공
    def _get_info(self):
        x, y, _ = self._state
        d = np.linalg.norm(self.goal - np.array([x, y]))    # 목표까지의 거리
    ...
    def step(self, action):
        ...
        x += math.cos(th) * self.v  # 전진
        y += math.sin(th) * self.v

        self._state = np.array([x, y, th], dtype=np.float32)  # 상태 업데이트

        goal_dist = np.linalg.norm(self._state[:2] - self.goal)
        process = self._prev_goal_dist - goal_dist  # 접근 변화량
        self._prev_goal_dist = goal_dist

        reward = 1.0 * process - 0.01   # 보상 계산 : 매 스텝마다 패널티 (-0.01) 부여 → 시간 낭비 방지
        terminated, truncated = False, False
        if goal_dist <= self.goal_radius:   # 목표 도달
            reward += 1.0
            terminated = True
        elif self._steps >= self.max_steps: # 스텝 초과로 종료
            truncated = True
        elif self._collision():
            reward -= 1.0
            terminated = True

        obs = self._get_obs()   # 라이더 거리 벡터
        info = self._get_info() # goal dist, steps
    ...
    def render(self):
        ...
        ax.set_title('Simple 2D LiDAR Env')
        ...
        obs, angs = cast_lidarFunc(x, y, th)    # 라이다 빔 시각화
        for d, a in zip(obs, angs):
            # 두 점을 연결하는 선 그리기
            ax.plot([x, x + np.cos(a) * d], [y, y + np.sin(a) * d], lw=1, alpha=0.9)

        ax.legend(loc='upper right')
        plt.pause(0.05)    # 프레임을 갱신, rander() 내에서 매번 그림이 다시 그려지게 하기 위함
...
if __name__ == "__main__":
    ...
    for t in range(500):
        action = env.action_space.sample() # 환경에서 가능한 행동범위 중 무작위로 행동 하나를 선택 (직진, 좌, 우)
        obs, reward, terminated, truncated, info = env.step(action)    # 환경 단계 실행 → 환경이 다음(새로운) 상태와 보상 등의 반환
        # ↪ obs : 새로운 상태, reward : 보상, terminated : 목표 도달/충돌 여부, truncated : 시간 초과 여부, info : 추가 정보
        total_reward += reward
        env.render()
