# pip install flask
from flask import Flask     # 웹 서버 생성에 필요함
# Python Application Server : py 프로그램 코드를 실행해서 요청을 처리하는 서버


# Flask 기본 서버는 실무용 아님. 개발용, 학습용
# 실무용 서버(MSGI) : gunicorn, waitress, nginx----
...
@app.route('/')             # URL 매핑 : 클라이언트 요청이 '/'일때 아래 함수 수행
def abc():                  # client 요청을 처리하는 함수
